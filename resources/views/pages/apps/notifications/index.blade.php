    ...
    <div class="card mb-8">
        <div class="card-body d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-4">
            <div>
                ...
                <div class="text-muted">Track billing reminders, viewer milestones, and direct admin notices in one
                    inbox.</div>
            </div>
            <div class="d-flex gap-3 align-items-center">
                <span class="badge badge-light-primary fs-6 px-4 py-3">Unread: {{ $unreadCount }}</span>
                @if ($unreadCount > 0)
                    <form method="POST" action="{{ route('notifications.read-all') }}">
                        @csrf
                        <button type="submit" class="btn btn-light-primary">Mark All Read</button>
                    </form>
                @endif
            </div>
        </div>
        ...
        <div class="card-body">
                    ...
                    <div class="d-flex align-items-start gap-4">
                        ...
                        <div>
                            ...
                            <div class="text-gray-700 mb-2">
                                {{ data_get($notification->data, 'message', 'Notification received.') }}</div>
                            <div class="text-muted fs-7">{{ $notification->created_at?->format('d M Y H:i') }} •
            ...
            @empty
                <div class="py-15 text-center">
                    <div class="text-gray-500 fs-5">You have no notifications yet.</div>
                </div>
            @endforelse
        </div>
